...
from .conf_types import CoreConfig, OfflineCoreConfig, OnlineCoreConfig, CoreConfInChip, CorePlmConfig, NeuronConfig
...
class CoreBlock(CoreAbstract):
    ...
    def max_lcn_ex(self) -> LCN_EX:
        return LCN_EX.LCN_64X if self.online else LCN_EX.LCN_8X
    ...
    def lcn_ex(self, lcn_ex: LCN_EX) -> None:
        ...
        if lcn_ex > self.max_lcn_ex:
            raise ResourceError(
                f"required LCN out of range {self.max_lcn_ex} ({lcn_ex})."
            )
    ...
    @classmethod
    def build_core_blocks(cls, msgrp: MergedSuccGroup, online: bool = False) -> list["CoreBlock"]:
        ...
        if online:
            def get_seed(node: SourceNodeType) -> int:
                raise NotImplementedError
            ...
            for idx_lst in idx_of_sg:
                ...
                seed = get_seed(succ_nodes[i])
                # Todo: Get seed from nodes here
                core_block = OnlineCoreBlock.build(*succ_edges, seed=seed)
            ...
            for idx_lst in idx_of_sg:
                ...
                core_block = OfflineCoreBlock.build(*succ_edges, rt_mode=mode)
                core_blocks.append(core_block)
...
class OnlineCoreBlock(CoreBlock):
    ...
    def _n_axon2lcn_ex(self) -> LCN_EX:
        ...
        if (
            ...
            _max_n_axons = self.n_fanin_base << LCN_EX.LCN_8X
            raise ResourceError(
                f"required LCN out of range {LCN_EX.LCN_8X} ({lcn}). The number of axons "
                f"must be <= {_max_n_axons}, but synapses {self._obj_repr} have a total of "
                f"{self.n_axon} axons."
            )
    ...
    def n_fanout(self) -> int:
        """The fan-out of cores."""
        return 1024 >> self.dendrite_comb_rate
    ...
    @classmethod
    def build(cls, *synapses: FullConnectedSyn, seed: int = 0):
        """Group synapses & build `CoreBlock`."""
        if seed > (1 << 64) - 1:
            warnings.warn(
                f"random seed {seed} is too large, truncated into 64 bits.",
...
class CorePlacement(CoreAbstract):
    ...
    def export_param_config(self) -> CoreConfig:
        ...
        # fmt: off
        cb_config = OfflineCoreConfig.build(
